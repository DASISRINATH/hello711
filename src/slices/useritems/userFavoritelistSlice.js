...
//Fetch All User Items
  export const fetchUserFavoriteListData = createAsyncThunk(
   ...
   try {
       const response = await axios(
        {
          method: "POST",
          url: `items/get_favourite/api_key/${process.env.REACT_APP_API_SECURITY_KEY}/login_user_id/usrfbceafda0e2617e6152cf48a8013d9e2/limit/10/offset/0`,
        }
       ); 
...
//Delete a User Item
  ...
  console.log(fetchUserFavoriteListData);
  const userItemSlice = createSlice({
    name: 'userItem',
    initialState:useritemsAdapter.getInitialState({loading:false}),
    reducer:{},
    extraReducers: {
      [fetchUserFavoriteListData.pending]: (state, action) => {
        state.loading = true;
        state.error = null;
      },
      [fetchUserFavoriteListData.fulfilled]: (state, {payload}) => {
        state.loading = false;
        useritemsAdapter.setAll(state,payload)
      },
      [fetchUserFavoriteListData.rejected]: (state, action) => {
