...
import LandDataFormModal from "./LandDataFormModal";
import "@geoman-io/leaflet-geoman-free";
import "@geoman-io/leaflet-geoman-free/dist/leaflet-geoman.css";
import axios from "axios";

export default function DrawControlComponent({drawControlOnMap, editControlOnMap, setLayoutId}){
  const map = useMap();
  ...
  useEffect(()=>{
    if(drawControlOnMap){
      ...
      map.pm.Toolbar.changeControlOrder(["drawPolygon", "drawRectangle"]);
      ...
      if (layersPlot.length) {
        ...
        for (var i = layersPlot.length - 1; i >= 0; i--) {
          ...
          layer.on("pm:remove", async (e) => {
            console.log(e);
            axios
              .delete(`https://webgis-web-app.herokuapp.com/layout/deletelayout/${e.layer.feature.properties.layoutId}`)
                .then((res) => res.data)
                .then((data) => {
                  console.log(data);
                })
